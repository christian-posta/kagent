...
func BuildSubstrateBootstrapJSON(ctx context.Context, kube client.Client, namespace string, sbx *v1alpha2.AgentHarness, mc *v1alpha2.ModelConfig, gw GatewayBootstrapConfig) ([]byte, []corev1.EnvVar, error) {
	...
	// kagent fork: channels (Telegram/Slack) intentionally dropped from this
	// fork's openclaw helpers — the substrate-demo path doesn't need them and
	// the upstream channels code depends on packages we don't carry here.
	// If you need channel support, restore channels_shared.go/channels_substrate.go
	// from pj-kagent + add the AgentHarnessOpenClawSlackOptions types.
	_ = ctx
	_ = kube
	_ = namespace
	_ = sbx
	var channelEnv []corev1.EnvVar
